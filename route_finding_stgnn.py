...
def prepare_input_data(temporal_df, start_time, window_size=48, num_nodes=41):
    ...
    input_tensor = np.concatenate([flow_values, imputed_values, time_features], axis=-1)  # Shape: (window_size, nodes, 5)
    return torch.FloatTensor(input_tensor).unsqueeze(0).to(device), scaler.transform(flow_matrix.values)

def predict_flows(model, input_tensor, static_adj, horizon=4, num_steps=12, mc_dropout_samples=10):
...
def route_finding_stgnn(spatial_file, temporal_file, model_path, start_scat, dest_scat, start_time_str, scaler_path=None):
    ...
    flows_pred, uncertainties_pred = predict_flows(model, input_tensor, static_adj_tensor, horizon=4, num_steps=12)
